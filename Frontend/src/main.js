...
const qrUrl = id => `${(data?.restaurant.publicBaseUrl || "https://qr-menu-demo-9vhn.onrender.com").replace(/\/$/, "")}/?scan=${encodeURIComponent(id)}`;
...
function guestItems(menu) {
  ...
  if (!categories.length) return `<div class="empty-state">${icon("items")}<h3>${guestQuery ? "No items found" : "Selections will appear here"}</h3><p>${guestQuery ? "Try another search." : "Please check with staff for available selections."}</p></div>`;
  return categories.map(category => `<section class="guest-category"><div class="category-title"><h3>${esc(category.name)} <span>${category.items.length}</span></h3>${category.description ? `<p>${esc(category.description)}</p>` : ""}</div><div class="dish-grid">${category.items.map(item => `<article class="dish-card"><div class="dish-copy">${item.vegetarian ? `<span class="vegetarian">${icon("leaf")} Vegetarian</span>` : ""}<h4>${esc(item.name)}</h4>${item.description ? `<p>${esc(item.description)}</p>` : ""}${item.allergens ? `<small>Contains: ${esc(item.allergens)}</small>` : ""}<strong>${money(item.price)}</strong></div>${item.imageUrl ? `<img class="dish-photo" src="${esc(item.imageUrl)}" alt="${esc(item.name)}" loading="lazy" referrerpolicy="no-referrer">` : `<div class="dish-placeholder">${icon("items")}</div>`}</article>`).join("")}</div></section>`).join("");
}
